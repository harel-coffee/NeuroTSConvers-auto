...
#==========================================================================================================================
def evaluate_features_2models (brain_areas, model1, model2, type, files_dir, alpha = 0.05):

    # load all behavioral data
    all_data = pd. read_pickle ("concat_time_series/behavioral_hh_data.pkl")


    df1 = pd. read_csv ("%s/%s_%s.csv"%(files_dir, model1, type), sep = ';')
    df2 = pd. read_csv ("%s/%s_%s.csv"%(files_dir, model2, type), sep = ';')

    results_df = []
    # Evaluate fscores of each brain area
    for br_area in brain_areas:

        best_results = df1. loc [df1. groupby ("region") ["fscore. mean"]. idxmax (), :]
        best_results = best_results. loc [best_results ["region"] == br_area, :]

        worst_results = df2. loc [df2. groupby ("region") ["fscore. mean"]. idxmax (), :]
        worst_results = worst_results. loc [worst_results ["region"] == br_area, :]
        ...
        results_df. append ([br_area, bestF_mean,  non_relatedF_mean, pvalue])
        #print ("\n", 25 * '=')

    return (pd. DataFrame (results_df, columns = ["ROIs", "Fscore %s"%model1, "Fscore %s"%model2, "T-test"]))
...
if __name__ == '__main__':
	...
	if args. baseline:
		...
		#df_hh = evaluate_features_2models (brain_areas, args. model, "baseline", type = "HH", files_dir = _files_dir)
		#df_hr = evaluate_features_2models (brain_areas, args. model,  "baseline", type = "HR", files_dir = _files_dir)
	...
	'''cols = list (df_hh. columns)[1:]
	print (cols)

	header1 = ["ROIS"] + ["Human-human" for i in range (len (cols))] + ["Human-machine" for i in range (len (cols))]
	header2 = ["ROIS"] + ["Best model", "Best model", "Baseline", "T-test"] +  ["Best model",  "Best model", "Baseline", "T-test"]
	header3 = ["ROIS"] + ["Classifer", "F-scores", "F-scores", "p-value"] +  ["Classifer", "F-scores", "F-scores", "p-value"]
	header4 = ["ROIS"] + cols + cols'''
