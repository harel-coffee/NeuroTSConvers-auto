...
if __name__ == '__main__':
	...
	# extract models filenames for the input conversat type (HH or HR)
	trained_models = glob. glob ("%s/trained_models/*"%(args.pred_module_path))
	trained_models = [a for a in trained_models if "_%s."%conversation_type in a]
	...
	for region in regions:
		...
		if fname == "":
			print ("Region %s has not been predicted"%region)
			continue

		model_name = fname. split ('/')[-1]. split ('_') [0]

		if "MLP_" in fname:
			model = MLP_MODEL (args.lag - 2)
			model. load (fname)

		elif "LSTM_" in fname:
			model = LSTM_MODEL (args.lag - 2)
			model. load (fname)
